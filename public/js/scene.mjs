    ...
    new THREE.ShaderMaterial({
      ...
      fragmentShader: /* glsl */`
        ...
        void main() {
          ...
          float g  = fbm5(p * 6.0 + vec3(uTime * 0.012, uTime * 0.008, -uTime * 0.010));
          float g2 = fbm(p * 16.0 - vec3(0.0, uTime * 0.02, uTime * 0.014));
          ...
          float spotN = fbm5(p * 1.35 + vec3(uSpotSeed) + vec3(uTime * 0.002));
          float spot = smoothstep(0.685, 0.72, spotN) * bandMask;
          // umbra (lõi rất tối) + penumbra (vành nâu loang quanh)
          float umbra = smoothstep(0.74, 0.78, spotN) * bandMask;
          float penumbra = spot * (1.0 - umbra);
          // ---- plage: vùng sáng từ trắng quanh ranh giới spot ----
          float plage = (smoothstep(0.60, 0.68, spotN) - smoothstep(0.685, 0.72, spotN)) * bandMask;
          // ---- palette nhiệt độ: granule sáng vàng-trắng, khe tối cam-đỏ ----
          float bright = g * 0.62 + g2 * 0.38;
          vec3 hot  = vec3(1.00, 0.96, 0.82);   // granule đỉnh ~6300K
          ...
          col = mix(col, vec3(0.42, 0.20, 0.07), penumbra * 0.85);
          col = mix(col, vec3(0.06, 0.02, 0.01), umbra * 0.95);
